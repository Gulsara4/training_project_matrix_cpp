...
TEST(test_class, basic_constructor) {
  S21Matrix m;
  EXPECT_EQ(m.GetCols_(), 1);
  EXPECT_EQ(m.GetRows_(), 1);
}

TEST(test_class, rc_constructor) {
  S21Matrix m(69, 420);
  EXPECT_EQ(m.GetRows_(), 69);
  EXPECT_EQ(m.GetCols_(), 420);
}
...
TEST(test_class, copy_constructor) {
  ...
  EXPECT_EQ(copy.GetCols_(), 12);
  EXPECT_EQ(copy.GetRows_(), 12);
}
...
TEST(test_class, move_constructor) {
  ...
  EXPECT_EQ(m.GetCols_(), 0);
  EXPECT_EQ(m.GetRows_(), 0);
  EXPECT_EQ(moved.GetCols_(), 12);
  EXPECT_EQ(moved.GetRows_(), 12);
}
...
TEST(test_overload, equal_lvalue) {
  ...
  EXPECT_EQ(m.GetRows_(), 3);
  EXPECT_EQ(m.GetCols_(), 6);
  EXPECT_EQ(m(1, 2), 3);
  EXPECT_EQ(m(2, 2), 3);
}
...
TEST(test_overload, equal_rvalue) {
  ...
  EXPECT_EQ(n.GetCols_(), 2);
  EXPECT_EQ(n.GetRows_(), 2);
}
...
TEST(test_functional, inverese_3x3) {
  ...
  given[2][1] = -3;
  given[2][2] = 2;
  given = (given.InverseMatrix());
  ASSERT_TRUE(given == expected);
}
